...
class Ckl_lost extends Ckl_Base{
    ...
    function getPageByIndex($page){
        return $this->mysql->queryAllFLimit(($page - 1)*PAGE_INDEX, ($page)*PAGE_INDEX);
    }
    ...
    function searchByMain(){
        $array = $this->mysql->searchByKey("main","18475098348");
        return $array();
    }

    function getSearchResByIndex($if,$index){
        $values["main"] = $if;
        $values["local"] = $if;
        $values["info"] = $if;
        $values["title"] = $if;
        $index -= 1;
        $array = $this->mysql->searchByKeyS($values);
        $resArray = array_slice($array,$index*PAGE_INDEX,($index+1)*PAGE_INDEX);
        return $resArray;
    }
